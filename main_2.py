...
async def upload_video(request: Request, video: UploadFile = File(...)):
    ...
    # 제한 조건 확인
    if video_duration > 30 or video_size > 50:
        cap.release()
    ...
    if not video.filename.endswith(('.mp4', '.mov')):
        ...
        try:
            subprocess.run(["ffmpeg", "-i", video_path, "-c:v", "libx264", "-c:a", "aac", "-strict", "experimental", converted_video_path], check=True)
...
async def select_model(
    ...
    FRAME_RATE_MAPPING = {
        'low': 5,    # 5번째 프레임마다 frame extracting (Hard)
        'medium': 15,  # 15번째 프레임마다 frame extracting (Medium)
        'high': 30    # 30번째 프레임마다 frame extracting (Easy)
    }
    frame_rate_value = FRAME_RATE_MAPPING.get(frame_rate)
    if frame_rate_value is None:
        raise HTTPException(status_code=400, detail="Invalid frame rate selected.")
...
async def results(request: Request, session_id: str):
    sessions = session_manager.sessions
    # 결과가 준비되었는지 확인
    if 'result_ready' not in sessions[session_id]:
